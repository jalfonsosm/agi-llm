...
/* Internal backend configuration (uses nagi_llm_config_t from public API) */
typedef nagi_llm_config_t llm_config_t;

static llm_state_t *g_llm_state = NULL;
static llm_config_t g_llm_config = {
    .backend = NAGI_LLM_BACKEND_LLAMACPP,
    .model_path = "",
    .context_size = NAGI_LLM_DEFAULT_CONTEXT_SIZE,
    .batch_size = NAGI_LLM_DEFAULT_BATCH_SIZE,
    .u_batch_size = NAGI_LLM_DEFAULT_U_BATCH_SIZE,
    .n_threads = NAGI_LLM_DEFAULT_THREADS,
    .temperature = 0.0f,  // Greedy decoding for deterministic matching
    .top_p = 0.9f,
    .top_k = 1,   // Only consider top token
    .max_tokens = 5,  // Only need "yes" or "no" for semantic, or "verb noun" for extraction
    .use_gpu = 1,
    .verbose = 0,
    .mode = NAGI_LLM_MODE_EXTRACTION // NAGI_LLM_MODE_SEMANTIC  // Default to extraction mode (faster)
};
...
int llm_parser_init_real(const char *model_path, const llm_config_t *config)
{
    ...
    if (model_path) {
        strncpy(g_llm_config.model_path, model_path, NAGI_LLM_MAX_MODEL_PATH - 1);
    }
 ...
 */
void llm_parser_shutdown_real(void)
{
 ...
 */
int llm_parser_ready_real(void)
{
...
static const char *get_word_string(int word_id)
{
    ...
    if (!g_llm_state || !g_llm_state->dictionary_data) {
    ...
    for (i = 0; i < 26; i++) {
        ...
        u16 offset = load_be_16(g_llm_state->dictionary_data + i * 2);

        /* Skip if no words for this letter */
        if (offset == 0) continue;

        /* Start at the word list for this letter */
        u8 *ptr = g_llm_state->dictionary_data + offset;
...
static const char *extract_game_verbs(void)
{
    ...
    if (!g_llm_state->dictionary_data) {
        fprintf(stderr, "LLM Parser: g_llm_state->dictionary_data not loaded, cannot extract verbs\n");
        return NULL;
    }
 ...
 */
const char *llm_parser_extract_words_real(const char *input)
{
    ...
    if (!llm_parser_ready_real()) return input;
    ...
    current_seq = (g_llm_state->seq_counter++) % 8;
 ...
 */
int llm_parser_matches_expected_real(const char *input,
                                ...
                                int expected_count)
{
    ...
    if (!llm_parser_ready_real()) return 0;
 ...
 */
int llm_parser_generate_response_real(const char *game_response, const char *user_input,
...
/*
 * ============================================================================
 * NAGI_LLM Interface Wrapper for llama.cpp backend
 * ============================================================================
 */

/* Forward declarations of original functions (now static/internal) */
extern int llm_parser_init_real(const char *model_path, const llm_config_t *config);
extern void llm_parser_shutdown_real(void);
extern int llm_parser_ready_real(void);
extern const char *llm_parser_extract_words_real(const char *input);
extern int llm_parser_matches_expected_real(const char *input, const int *expected_word_ids, int expected_count);
extern int llm_parser_generate_response_real(const char *game_response, const char *user_input, char *output, int output_size);

/* Adapter functions */
static int llamacpp_init(nagi_llm_t *self, const char *model_path, const nagi_llm_config_t *config) {
    (void)self;
    /* Convert nagi_llm_config_t to llm_config_t */
    llm_config_t internal_config;
    if (config) {
        strncpy(internal_config.model_path, config->model_path, sizeof(internal_config.model_path)-1);
        internal_config.context_size = config->context_size;
        internal_config.batch_size = config->batch_size;
        internal_config.u_batch_size = config->u_batch_size;
        internal_config.n_threads = config->n_threads;
        internal_config.temperature = config->temperature;
        internal_config.top_p = config->top_p;
        internal_config.top_k = config->top_k;
        internal_config.max_tokens = config->max_tokens;
        internal_config.use_gpu = config->use_gpu;
        internal_config.verbose = config->verbose;
        internal_config.mode = (nagi_llm_mode_t)config->mode;
    }
    return llm_parser_init_real(model_path, config ? &internal_config : NULL);
}

static void llamacpp_shutdown(nagi_llm_t *self) {
    (void)self;
    llm_parser_shutdown_real();
}

static int llamacpp_ready(nagi_llm_t *self) {
    (void)self;
    return llm_parser_ready_real();
}

static const char *llamacpp_extract_words(nagi_llm_t *self, const char *input) {
    (void)self;
    return llm_parser_extract_words_real(input);
}

static int llamacpp_matches_expected(nagi_llm_t *self, const char *input,
                                     const int *expected_word_ids, int expected_count) {
    (void)self;
    return llm_parser_matches_expected_real(input, expected_word_ids, expected_count);
}

static int llamacpp_generate_response(nagi_llm_t *self, const char *game_response,
                                      const char *user_input, char *output, int output_size) {
    (void)self;
    return llm_parser_generate_response_real(game_response, user_input, output, output_size);
}

/*
 * Wrapper function for set_dictionary vtable entry
 */
static int llamacpp_set_dictionary(nagi_llm_t *self, const unsigned char *dictionary, size_t size) {
    (void)self;  /* Currently using global state */
    ...
    g_llm_state->dictionary_data = dictionary;
    g_llm_state->dictionary_size = size;
...
nagi_llm_t *nagi_llm_llamacpp_create(void) {
    nagi_llm_t *llm = (nagi_llm_t *)malloc(sizeof(nagi_llm_t));
    if (!llm) {
        return NULL;
    }
    ...
    llm->impl_data = NULL;  /* Using global state for now */
