...
void llm_context_add(llm_context_type_t type, const char *text)
{
    ...
    entry->type = type;
    entry->timestamp = state.ticks;
    entry->room = state.var[V00_ROOM0];
...
void llm_context_track_flag(int flag_num, const char *description)
{
    ...
    int idx = g_llm_context.tracked_flags_count++;
...
void llm_context_update_flags(void)
{
    for (int i = 0; i < g_llm_context.tracked_flags_count; i++) {
        int flag_num = g_llm_context.tracked_flags[i].flag_num;
        int byte_idx = flag_num / 8;
        int bit_idx = flag_num % 8;
        g_llm_context.tracked_flags[i].value = (state.flag[byte_idx] >> bit_idx) & 1;
    }
    g_llm_context.context_dirty = 1;
}
